...
cmd="THEANO_FLAGS=mode=FAST_RUN,device=gpu$gpu_id,floatX=float32 python -u $run_file -neg_samples $neg_samples -num_entities $ent -num_relations $rels -num_entity_pairs $ent_pairs  -model $model -dataset /home/prachij/code/joint_embedding/DATA_REPOSITORY/$dataset/$file_dir -l2_entity $l2_entity -l2_relation $l2_relation_DM -batch_size $batch_size -epochs $num_epochs -eval_every $eval_every -eval_after $eval_after -train $train -oov_avg $oov_avg -oov_train $oov_train -unit_norm $unit_norm_reg -rate $learning_rate -vect_dim $vect_dim -shared_r $shared_r -l2_entity_pair $l2_entity_pair -static_alpha $static_alpha -alphaMF $alphaMF -evalDev $evalDev -l2_relation_MF $l2_relation_MF -add_loss $add_loss -oov_eval $oov_eval -loss $loss -init_model $init_model -theo_reg $theo_reg -log_file $log_file"
echo $cmd

THEANO_FLAGS=mode=FAST_RUN,device=gpu$gpu_id,floatX=float32 python -u $run_file -neg_samples $neg_samples -num_entities $ent -num_relations $rels -num_entity_pairs $ent_pairs  -model $model -dataset /home/prachij/code/joint_embedding/DATA_REPOSITORY/$dataset/$file_dir -l2_entity $l2_entity -l2_relation $l2_relation_DM -batch_size $batch_size -epochs $num_epochs -eval_every $eval_every -eval_after $eval_after -train $train -oov_avg $oov_avg -oov_train $oov_train -unit_norm $unit_norm_reg -rate $learning_rate -vect_dim $vect_dim -shared_r $shared_r -l2_entity_pair $l2_entity_pair -static_alpha $static_alpha -alphaMF $alphaMF -evalDev $evalDev -l2_relation_MF $l2_relation_MF -add_loss $add_loss -oov_eval $oov_eval -loss $loss -init_model $init_model -theo_reg $theo_reg -model_path $model_path &> $3
