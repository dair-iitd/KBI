...
def get_params():
    ...
    parser.add_argument('-num_entities', action="store", default=14951, dest="num_entities", type=int)
    ...
    parser.add_argument('-dev_folder', action="store", default="valid_tensor", dest="dev_folder", type=str)
...
def generate_valid_negSamples(opts):
    dataPath = opts.path
    if opts.test_on_valid:
        print "Encoding Vaidation data"
        test_data, _, _ = get_data(dataPath+"/encode_valid.txt")
        ...
        test_data, _, _ = get_data(dataPath+"/encode_test.txt")
        folderName = opts.test_folder

    total_entities = opts.num_entities #this includes ALL the entities-> TRAIN + VALID + TEST
    neg_samples = opts.neg_samples

    neg_data = np.array(gen_neg_samples(test_data, neg_samples, total_entities), dtype='int32')
    dataPath += "/"+folderName

    if (opts.test_on_valid):
        joblib.dump(neg_data, dataPath+"/valid_neg_samples.joblib")
    else:
        joblib.dump(neg_data, dataPath+"/test_neg_samples.joblib")
...
if __name__ == '__main__':
    ...
    generate_valid_negSamples(opts)
